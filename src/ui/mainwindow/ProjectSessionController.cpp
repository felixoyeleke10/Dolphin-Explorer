...
bool ProjectSessionController::ensureImportsIdle(const QString& dialog_title)
{
    ...
    // Imports hold project-owned state, so the transition must not proceed
    // while they run — but the operator still needs an exit that is not Task
    // Manager. Offer the honest escape: drop the queued files and let the one
    // currently decoding settle (in-flight decodes have no cancellation
    // token), then proceed only once the state is actually released.
    ...
    if (waitForImportsToSettle(20000))
        return true;
...
bool ProjectSessionController::waitForImportsToSettle(int timeout_ms)
{
    ...
    progress.setCancelButton(nullptr);       // the bounded timeout is the exit
    progress.setMinimumDuration(300);        // skip the flash for fast settles
    ...
    poll.start();
    loop.exec();   // pumps queued job-completion signals while we wait
