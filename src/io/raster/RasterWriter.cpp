...
void applyGeoref(GDALDataset* ds, const double gt[6], const std::string& wkt)
{
                           ...
                           std::string* err)
{
    ...
    GDALDataset* ds = drv->Create(path.c_str(),
                                  static_cast<int>(grid.cols),
                                  static_cast<int>(grid.rows),
                                  1, GDT_Float32, opts);
    CSLDestroy(opts);
    if (!ds) { setErr(err, "Could not create the output GeoTIFF."); return false; }

    applyGeoref(ds, grid.geo_transform, grid.crs_wkt);

    GDALRasterBand* band = ds->GetRasterBand(1);
    band->SetNoDataValue(grid.no_data_value);
    ...
    GDALClose(ds);
    if (e != CE_None) { setErr(err, "GDAL failed to write raster pixels."); return false; }
    return true;
}
                       ...
                       std::string* err)
{
    ...
    applyGeoref(ds, geo_transform, crs_wkt);
    ...
    GDALClose(ds);
    if (!ok) { setErr(err, "GDAL failed to write image pixels."); return false; }
    return true;
}
